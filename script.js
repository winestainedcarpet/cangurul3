...
function drawCangur() {
   if (grid_type_select.value == "disc") {
      cangurElement.style.setProperty(
         "transform",
         `rotate(${cangur.segment * (360 / Number(segments_input.value))}deg) translateX(${cangur.ring * 32}px)`
      )
...
function resetDrawing() {
   ...
   if (grid_type_select.value == "square") {
      ...
      for (let i = 0; i < segments; i++) {
         ...
         ctx.lineTo(
            center + Math.sin(angle + 0.5 * Math.PI) * radius,
            center + Math.cos(angle + 0.5 * Math.PI) * radius
         )
         ctx.stroke()
      }
   }
